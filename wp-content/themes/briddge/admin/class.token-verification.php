...
class Zozo_Purchase_Code_Verification {
	...
	public $product_id = '39169028'; //Bridge Multipurpose Theme
	...
	public function token_api_call( $email, $purchase_code ) {
		...
		if ( 200 !== $response_code ) {
			return new WP_Error( $response_code, esc_html__( 'An unknown API error occurred.', 'briddge' ) );
		}
		$data = wp_remote_retrieve_body( $response ); //json_decode( wp_remote_retrieve_body( $response ), true );
		
		if ( null === $data ) {
			return new WP_Error( 'api_error', esc_html__( 'An unknown API error occurred.', 'briddge' ) );
		}
	...
	public function verify_token(){
		...
		$email = $_POST['zozo_registration_email'];
		if ( empty( $email ) ) {
			return array( 'error_message' => esc_html__( '!Oops.. Email id is required.', 'briddge' ) );
		}elseif( !is_email( $email ) ){
			return array( 'error_message' => esc_html__( '!Oops.. Email id is invalid.', 'briddge' ) );
		}
	
		$purchase_code = sanitize_text_field( $_POST['zozo_purchase_code'] );
		if ( empty( $purchase_code ) ) {
			return array( 'error_message' => esc_html__( '!Oops.. Purchase code is required.', 'briddge' ) );
		}
