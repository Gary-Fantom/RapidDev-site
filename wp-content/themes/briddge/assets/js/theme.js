...
(function( $ ) {
		...
		if( $('.zozo-mc').length ){
			$('.zozo-mc').on( "click", function () {
				...
				if( mc_form.find('input[name="zozo_mc_email"]').val() == '' ){
					mc_wrap.find('.mc-notice-msg').text( briddge_ajax_var.must_fill );
				}else{
					c_btn.attr( "disabled", "disabled" );
					$.ajax({
						type: "POST",
						url: briddge_ajax_var.ajax_url,
						data: 'action=zozo-mc&nonce='+briddge_ajax_var.mc_nounce+'&'+mc_form.serialize(),
	...
	$( window ).load(function() {
		...
		//Briddge masonry
		if( $(".briddge-masonry").length ){
			var masonry_on_time;			
			var masonry_cols = $(".briddge-masonry").data('columns');			
			var masonry_gutter = $(".briddge-masonry").data('gutter');
			
			$(".briddge-masonry").rcmasonry({
    ...
    //Elementor RTL issue
    function briddge_for_elementor_row(){
    ...
    $(window).resize(function(event){
		...
		if( $(".sticky-outer").length ){
			$(".sticky-outer").each(function(){
				...
				_sticky_ele.briddge_stickypart();
				var sticky_on_time;
				$(window).resize(function(event){
					clearTimeout(sticky_on_time);
					sticky_on_time = setTimeout(function(){ _sticky_ele.briddge_stickypart(); }, 300);
				});
			});
		}
...
(function ( $ ) {
	...
	//Make sticky
	$.fn.briddge_stickypart = function( options ){
