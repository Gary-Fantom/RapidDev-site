...
class CEA_Elementor_Toggle_Content_Widget extends Widget_Base {
	...
	protected function register_controls() {
		...
		$this->start_controls_section(
			"general_section",
			[
				"label"	=> esc_html__( "Toggle", "cea" ),
				"tab"	=> Controls_Manager::TAB_CONTENT,
				"description"	=> esc_html__( "Default toggle content options.", "cea" ),
			]
		);
		$this->add_control(
			"tg_height",
			[
				...
				"label"			=> esc_html__( "Number of Lines Toggle", "cea" ),
				"description"	=> esc_html__( "Here you put the number of lines want to show on page load. Other lines are hidden untill when trigger toggle button.", "cea" ),
				"default" 		=> "10",
				'placeholder' 	=> "10"
			]
		);
		$this->add_control(
			"tg_content",
			[
				"label"			=> esc_html__( "Toggle Content", "cea" ),
		...
		$this->start_controls_tab(
			'tab_slide_up',
			[
				'label' => esc_html__( 'Toggle Inactive', 'cea' ),
			]
		);
		$this->add_control(
			'button_text_up',
		...
		$this->start_controls_tab(
			'tab_slide_down',
			[
				'label' => esc_html__( 'Toggle Active', 'cea' ),
			]
		);
		$this->add_control(
			'button_text_down',
	...
	protected function render() {
		...
		$btn_text = isset( $btn_text ) && !empty( $btn_text ) ? '<span class="toggle-btn-txt">'. $btn_text .'</span>' : '<span class="toggle-btn-txt">'. esc_html__( 'More', 'cea' ) .'</span>';
		$btn_less_text = isset( $btn_less_text ) && !empty( $btn_less_text ) ? $btn_less_text : esc_html__( 'Less', 'cea' );
