...
class Zozothemes_API {
	...
	public function request( $url ) {
		...
		if ( 200 !== $response_code ) {
			return new WP_Error( $response_code, esc_html__( 'An unknown API error occurred.', 'cea' ) );
		}
		$body_data = json_decode( wp_remote_retrieve_body( $response ), true );
		if ( null === $body_data ) {
			return new WP_Error( 'api_error', esc_html__( 'An unknown API error occurred.', 'cea' ) );
		}
