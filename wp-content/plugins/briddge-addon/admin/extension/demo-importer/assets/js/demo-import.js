...
( function( $ ) {
	...
	$(document).on( 'click', '.button-uninstall-demo', function(e) {
		...
		$.confirm({
			theme: 'supervan',
			title: false,
			content: briddge_admin_ajax_var.unins_confirm,
			confirmButtonClass: 'btn-success',
			cancelButtonClass: 'btn-danger',
			confirmButton: 'Uninstall',//briddge_admin_ajax_var.yes,
   			cancelButton: briddge_admin_ajax_var.no,
			confirm: function(){
				...
				$(progress).prev(".progress-text").text(briddge_admin_ajax_var.uninstalling + '..');
				loading_wrap.show();
				...
				$.ajax({
					...
					url: ajaxurl,
					data: {
						action: 'briddge_demo_import',
						process: 'uninstall',
						nonce:  $("#briddge_demo_import_nonce").val()
					},
					success: function(response){
						if( response && response.indexOf('success') == -1 ) {
							...
							loading_wrap.hide();
							progress.parent(".installation-progress").hide();
							progress_text.text(briddge_admin_ajax_var.uninstalled);
					...
					error: function(response, errorThrown){
						loading_wrap.hide();
						progress.parent(".installation-progress").hide();
						alert(briddge_admin_ajax_var.unins_pbm);
					}
				});
			}
		});
			...
			$(document).on( 'click', '.button-install-demo', function(e) {
				...
				if( choosed_demo !== null ) {
					...
					$.confirm({
						...
						confirmButtonClass: 'btn-success',
    					cancelButtonClass: 'btn-danger',
						confirmButton: briddge_admin_ajax_var.proceed,
   						cancelButton: briddge_admin_ajax_var.cancel,
						confirm: function(){
							...
							loading_wrap.show();
							$(current).parents('.zozothemes-demo-item').find('.installation-progress p').text(briddge_admin_ajax_var.downloading);
							$('.zozo-importer-notice').hide();
						
							// Demo Files Download
							briddge_check_file_access_permission( choosed_demo, revslider, progress );
	...
	function briddge_progress_update( progress ){
	...
	function briddge_progress_end( choosed_demo, progress ){
		...
		$.confirm({
			...
			confirmButton: 'Ok',
			cancelButton: 'Close',
			confirm: function(){ window.location = briddge_admin_ajax_var.briddge_import_url; }
		});
	}
	
	function briddge_progress_details( progress, msg, stat ){
	...
	function briddge_check_file_access_permission( choosed_demo, revslider, progress ){
		$.ajax({
			type: 'post',
			dataType: 'json',
			url: ajaxurl,
			...
			success: function(response){
				if( response['msg'] == 'failed' ) {
					...
					briddge_progress_details( progress, response['msg'], '' );
					briddge_progress_end( choosed_demo, progress );					
				}else{
					briddge_general_file_download_function( 0, choosed_demo, revslider, progress );
				}
			},
			error: function(response, errorThrown){
				 
				briddge_progress_details( progress, response['msg'], 'failed' );
				briddge_progress_end( choosed_demo, progress );
			}
		});//ajax end		
	}
	
	function briddge_general_file_download_function( index, choosed_demo, revslider, progress ){
		...
		if( max_len > index ){
			$.ajax({
				...
				success: function(response){
					...
					if( max_len > index ){
						briddge_general_file_download_function( index, choosed_demo, revslider, progress );
					}else{
						briddge_page_file_download_function( 0, choosed_demo, revslider, progress );
					}
					briddge_progress_update( progress );
				},
				error: function(response, errorThrown){
					briddge_progress_details( progress, response['msg'], 'failed' );
					index += 1;
	...
	function briddge_page_file_download_function( index, choosed_demo, revslider, progress ){
		...
		if( max_len > index ){
			$.ajax({
				...
				success: function(response){
					...
					if( max_len > index ){
						briddge_page_file_download_function( index, choosed_demo, revslider, progress );
					}else{
						briddge_general_file_install_function( 0, choosed_demo, revslider, progress );
					}
					briddge_progress_update( progress );
				},
				error: function(response, errorThrown){
		...
		if( max_len == 0 ){
			briddge_general_file_install_function( index, choosed_demo, revslider, progress );
		}
		
	}
	
	function briddge_general_media_install_function( index, choosed_demo, revslider, progress, parent_index ){
		...
		if( max_len > index ){
			$.ajax({
				...
				success: function(response){
					...
					index += 1;
					if( max_len > index ){
						briddge_general_media_install_function( index, choosed_demo, revslider, progress, parent_index );
					}else{
						briddge_progress_update( progress );
						parent_index += 1;
						briddge_page_file_install_function( 0, choosed_demo, revslider, progress );
					}
				},
				error: function(response, errorThrown){
					var msg = general_arr[index] + ' not imported';
					briddge_progress_details( progress, msg, 'failed' );
					index += 1;
					if( max_len > index ){
						briddge_general_media_install_function( index, choosed_demo, revslider, progress, parent_index );
					}else{
	...
	function briddge_page_file_install_function( index, choosed_demo, revslider, progress ){
		var parent_index = 0;
		var max_len = page_arr.length;
		if( max_len > index ){
			$.ajax({
				...
				success: function(response){
					...
					index += 1;
					if( max_len > index ){
						briddge_page_file_install_function( index, choosed_demo, revslider, progress );
					}else{
						parent_index = media_parts ? 1 : 0;
						briddge_general_file_install_function( parent_index, choosed_demo, revslider, progress );
					}
					briddge_progress_update( progress );
				},
				error: function(response, errorThrown){
	...
	function briddge_general_file_install_function( index, choosed_demo, revslider, progress ){
		var max_len = general_arr.length;
		if( max_len > index ){
		
			if( general_arr[index] == 'media' ){
				briddge_general_media_install_function( 0, choosed_demo, revslider, progress, index );
			}else{
				...
				$.ajax({
					...
					success: function(response){
						...
						if( max_len > index ){
							...
							briddge_import_final( choosed_demo, progress );
						}
						briddge_progress_update( progress );
					},
					error: function(response, errorThrown){
	...
	function briddge_import_final( choosed_demo, progress ){
		$.ajax({
			type: 'post',
			dataType: 'json',
			url: ajaxurl,
			...
			success: function(response){
				...
				$.confirm({
					...
					confirmButtonClass: 'btn-success',
					cancelButtonClass: 'btn-danger',
					confirmButton: briddge_admin_ajax_var.yes,
					cancelButton: briddge_admin_ajax_var.no,
					confirm: function(){
						var win = window.open( briddge_admin_ajax_var.regenerate_thumbnails_url , '_blank');
						if (win) {
							win.focus();
						}
						window.location = briddge_admin_ajax_var.briddge_import_url;
					},
					cancel: function(){
