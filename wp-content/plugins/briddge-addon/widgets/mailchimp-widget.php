<?php
class briddge_mailchimp_widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_ops = array( 'classname' => 'briddge_mailchimp_widget', 'description' => esc_html__('Mailchimp Widget', 'briddge-addon') );
		/* Widget control settings. */
		$control_ops = array( 'width' => 250, 'height' => 350, 'id_base' => 'briddge_mailchimp_widget' );
		
		$this->default_failure_message = esc_html__('There was a problem processing your submission.', 'briddge-addon');
		$this->default_signup_text = esc_html__('Join now!', 'briddge-addon');
		$this->default_success_message = esc_html__('Thank you for joining our mailing list. Please check your email for a confirmation link.', 'briddge-addon');
		$this->default_title = esc_html__('Sign up for our mailing list.', 'briddge-addon');
		$this->api_key = Briddge_Theme_Option::briddge_options( 'mailchimp-api' );
		
		/* Create the widget. */
		parent::__construct( 'briddge_mailchimp_widget', esc_html__( 'Briddge Mailchimp', 'briddge-addon' ), $widget_ops, $control_ops );
	}
	/**
		...
		<div class="mailchimp-wrapper">
			<form class="zozo-mc-form" id="<?php echo 'zozo-mc-form'; ?>" method="post">
				...
				<div class="form-group">
					<input type="text" placeholder="<?php esc_html_e('First Name', 'briddge-addon'); ?>" class="form-control first-name" name="zozo_mc_first_name" />
				</div>
				<?php
					}
				...
				?>	
				<div class="form-group">
					<input type="text" placeholder="<?php esc_html_e('Last Name', 'briddge-addon'); ?>" class="form-control last-name" name="zozo_mc_last_name" />
				</div>
				<?php	
					}
				?>
				<input type="hidden" name="briddge_mc_listid" value="<?php echo stripslashes( $instance['current_mailing_list'] ); ?>" />
				
				<div class="input-group">
					<input type="text" class="form-control zozo-mc-email" required="required" id="zozo-mc-email" placeholder="<?php esc_html_e('Email Address', 'briddge-addon'); ?>" name="zozo_mc_email">
			...
			<?php
				$success = $instance['success_message'] && $instance['success_message'] != '' ? $instance['success_message'] : esc_html__( 'Success', 'briddge-addon' );
				$fail = $instance['failure_message'] && $instance['failure_message'] != '' ? $instance['failure_message'] : esc_html__( 'Failed', 'briddge-addon' );
			?>
	...
	function form( $instance ) {
		...
		$defaults = array( 'title' => '', 'current_mailing_list' => '', 'signup_text' => '', 'collect_first' => '', 'collect_last' => '', 'subtitle' => '', 'success_message' => esc_html__('Success.', 'briddge-addon'), 'failure_message' => esc_html__('Failure.', 'briddge-addon'));
		...
		?>
		<h3><?php echo  esc_html__('General Settings', 'briddge-addon');?></h3>
		<!-- Widget Title: Text Input -->
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'zozo-mc-title' ) ); ?>"><?php esc_html_e('Title:', 'briddge-addon'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id('current_mailing_list') ); ?>"><?php echo esc_html__('Select a Mailing List :', 'briddge-addon'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id('signup_text') ); ?>"><?php echo esc_html__('Sign Up Button Text :', 'briddge-addon'); ?></label>
		...
		<h3><?php echo esc_html__('Personal Information', 'briddge-addon'); ?></h3>
		<p>
			...
			<label for="<?php echo esc_attr( $this->get_field_id('collect_first') ); ?>"><?php echo esc_html__('Collect first name.', 'briddge-addon'); ?></label>
			<br />
			...
			<label><?php echo esc_html__('Collect last name.', 'briddge-addon'); ?></label>
		</p>
		<h3><?php echo esc_html__('Notifications', 'briddge-addon'); ?></h3>
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id('subtitle') ); ?>"><?php echo esc_html__('Sub Title:', 'briddge-addon'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id('success_message') ); ?>"><?php echo esc_html__('Success Message:', 'briddge-addon'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id('failure_message') ); ?>"><?php echo esc_html__('Failure Message:', 'briddge-addon'); ?></label>
...
if( ! function_exists('briddge_mailchimp') ) {
	function briddge_mailchimp(){
				
		$nonce = $_POST['nonce'];
	  
		if ( ! wp_verify_nonce( $nonce, 'briddge-mailchimp' ) )
			die ( esc_html__( 'Busted', 'briddge-addon' ) );
		
		$list_id = isset( $_POST['briddge_mc_listid'] ) ? $_POST['briddge_mc_listid'] : '';
		...
		$success = isset( $_POST['briddge_mc_success'] ) ? $_POST['briddge_mc_success'] : '';
		...
		$api_key = Briddge_Theme_Option::briddge_options( 'mailchimp-api' );
	...
	add_action('wp_ajax_nopriv_zozo-mc', 'briddge_mailchimp');
	add_action('wp_ajax_zozo-mc', 'briddge_mailchimp');
}

//Mailchimp Get list id's
if( ! function_exists('briddge_get_mailchimp_list_ids') ) {
	function briddge_get_mailchimp_list_ids(){
