<?php
class briddge_contact_infos_widget extends WP_Widget {
	/**
	 * Widget setup.
	 */
	function __construct() {
		
		//add_action( 'admin_enqueue_scripts', array( $this, 'briddge_contact_infos_widget_script' ) );
		add_action( 'load-widgets.php', array( &$this, 'briddge_contact_infos_widget_script' ) );
		
		/* Widget settings. */
		$widget_ops = array( 'classname' => 'briddge_contact_info_widget', 'description' => esc_html__('A widget that displays an About widget', 'briddge-addon') );
		$control_ops = array( 'width' => 250, 'height' => 350, 'id_base' => 'briddge_contact_info_widget' );
		parent::__construct( 'briddge_contact_info_widget', esc_html__('Briddge Contact Info', 'briddge-addon'), $widget_ops, $control_ops );
		
	}
	
	function briddge_contact_infos_widget_script(){
		wp_enqueue_style( 'wp-color-picker' );
        wp_enqueue_script( 'wp-color-picker' );
	}
	
	public function briddge_shortcode_rand_id(){
	...
	function widget( $args, $instance ) {
		...
		$shortcode_css = '';
		$shortcode_rand_id = $rand_class = 'shortcode-rand-' . $this->briddge_shortcode_rand_id();
		...
		if( $shortcode_css ) $class .= ' ' . $shortcode_rand_id . ' briddge-inline-css';
		...
		?>
			...
			<div class="contact-widget widget-content">
				...
				<div class="contact-widget-info">
					<?php if($caddress) : ?>
					<p class="contact-address"><span class="bi bi-map"></span><span><?php echo wp_kses_post($caddress); ?></span></p>
					<?php endif; ?>	
					
					<?php if($cphone) : ?>
					<p class="contact-phone"><span class="bi bi-phone"></span><span><a href="tel:<?php echo preg_replace('/\s+/', '', $cphone); ?>"><?php echo esc_html($cphone); ?></a></span></p>
					<?php endif; ?>	
	...
	function form( $instance ) {
		...
		$defaults = array( 'title' => esc_html__( 'Contact Info', 'briddge-addon' ), 'ctext' => '', 'caddress' => '', 'cphone' => '', 'cmail' => '', 'font_color' => '', 'cfshortcode' => '', 'background_color' => '', 'background_img' => '' );
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'ctext' ) ); ?>"><?php esc_html_e( 'Text Message', 'briddge-addon' ); ?></label>
		...
		<!-- Address -->
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'caddress' ) ); ?>"><?php esc_html_e( 'Address', 'briddge-addon' ); ?></label>
		...
		<!-- Phone Numbers -->
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'cphone' ) ); ?>"><?php esc_html_e( 'Phone Numbers', 'briddge-addon' ); ?></label>
		...
		<!-- Email -->
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'cmail' ) ); ?>"><?php esc_html_e( 'Email', 'briddge-addon' ); ?></label>
		...
		<!-- Shortcode -->
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'cfshortcode' ) ); ?>"><?php esc_html_e( 'Contact Form Shortcode', 'briddge-addon' ); ?></label>
		...
		<!-- Font Color -->		
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'font_color' ) ); ?>"><?php esc_html_e( 'Font Color', 'briddge-addon' ); ?></label>
		...
		<!-- Background Color -->
		<p>
            <label for="<?php echo $this->get_field_id( 'background_color' ); ?>"><?php _e( 'Background Color', 'briddge-addon' ); ?></label>
            <span><?php _e( 'Choose widget background color', 'briddge-addon' ); ?></span>
		...
		<!-- Background Image -->
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'background_img' ) ); ?>"><?php esc_html_e( 'Background Image', 'briddge-addon' ); ?></label>
