<?php
class briddge_author_widget extends WP_Widget {
	/**
	 * Widget setup.
	 */
	function __construct() {
		/* Widget settings. */
		$widget_ops = array( 'classname' => 'zozo_author_widget', 'description' => esc_html__('A widget that displays authors details.', 'briddge-addon') );
		/* Widget control settings. */
		$control_ops = array( 'width' => 250, 'height' => 350, 'id_base' => 'zozo_author_widget' );
		/* Create the widget. */
		parent::__construct( 'zozo_author_widget', esc_html__('Briddge Author', 'briddge-addon'), $widget_ops, $control_ops );
	}
	/**
					...
					foreach ( $blogusers as $user ) {
						...
						$out .= '<li class="clearfix media">
							...
							<div class="author-meta media-body">
								...
								<p class="author-counts"><span><a href="'. esc_url( $author_link ) .'" rel="bookmark">' . count_user_posts( $user->ID ) .' '. esc_html__('Posts', 'briddge-addon') .'</a></span><span>'. $comment_count .' '. esc_html__('Comments', 'briddge-addon') .'</span></p>
	...
	function form( $instance ) {
		...
		<!-- Category -->
		<p>
		<label for="<?php echo esc_attr( $this->get_field_id('listby') ); ?>"><?php esc_html_e('List by:', 'briddge-addon') ?></label> 
		<select id="<?php echo esc_attr( $this->get_field_id('listby') ); ?>" name="<?php echo esc_attr( $this->get_field_name('listby') ); ?>" class="widefat categories" style="width:100%;">
			<option value='default' <?php if ('default' == $instance['listby']) echo 'selected="selected"'; ?>><?php esc_html_e('Default', 'briddge-addon') ?></option>
			<option value='post' <?php if ('post' == $instance['listby']) echo 'selected="selected"'; ?>><?php esc_html_e('No.of Post', 'briddge-addon') ?></option>
		</select>
		</p>
		
		<!-- Filter by -->
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'filter_by' ) ); ?>"><?php esc_html_e('Filter by:', 'briddge-addon'); ?></label>
			...
			<br><small><?php esc_html_e('Filter by - separate by comma(,) eg: administrator,author,editor,subscriber', 'briddge-addon'); ?></small>
		</p>
		
		<!-- Number of posts -->
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'number' ) ); ?>"><?php esc_html_e('Number of author to show:', 'briddge-addon'); ?></label>
