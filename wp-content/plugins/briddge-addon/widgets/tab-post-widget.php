<?php
class briddge_advance_tab_post_widget extends WP_Widget {
	/**
	 * Widget setup.
	 */
	function __construct() {
		/* Widget settings. */
		$widget_ops = array( 'classname' => 'zozo_advance_tab_post_widget', 'description' => esc_html__('A widget that displays your post by category on tab', 'briddge-addon') );
		/* Widget control settings. */
		$control_ops = array('id_base' => 'zozo_advance_tab_post_widget' );
		/* Create the widget. */
		parent::__construct( 'zozo_advance_tab_post_widget', esc_html__('Briddge Tab Post Widget', 'briddge-addon'), $widget_ops, $control_ops );
	}
	/**
				...
				<div role="tabpanel" class="tab-pane fade show active" id="tab1_<?php echo esc_attr( $widg_id ); ?>">
					<?php 
						if($selpost1 == 'recent'){
							...
							$query = array( 'posts_per_page' => $number1 ? $number1 : 3, 'meta_key' => 'briddge_post_views_count', 'orderby' => 'meta_value_num', 'order' => 'DESC', 'cat' => $categories1  );	
							$output = $this->advanceTabWidgetPostOutput($query);
				...
				<div role="tabpanel" class="tab-pane fade" id="tab2_<?php echo esc_attr( $widg_id ); ?>">
					<?php
						if($selpost2 == 'recent'){
							...
							$query = array( 'posts_per_page' => $number2 ? $number2 : 3, 'meta_key' => 'briddge_post_views_count', 'orderby' => 'meta_value_num', 'order' => 'DESC', 'cat' => $categories2  );	
							$output = $this->advanceTabWidgetPostOutput($query);
				...
				<div role="tabpanel" class="tab-pane fade" id="tab3_<?php echo esc_attr( $widg_id ); ?>">
					<?php 
						if($selpost3 == 'recent'){
							...
							$query = array( 'posts_per_page' => $number3 ? $number3 : 3, 'meta_key' => 'briddge_post_views_count', 'orderby' => 'meta_value_num', 'order' => 'DESC', 'cat' => $categories3  );	
							$output = $this->advanceTabWidgetPostOutput($query);
		...
		$tab1 = '<ul class="post-newsfeed">';										
			...
			if ($loop->have_posts()) {							
				while ($loop->have_posts()) { 
					...
					$tab1 .= '<p class="post-item-comment"><a href="'.get_comments_link(get_the_ID()).'" ><span>'. $comments_count->total_comments .' '. esc_html__( 'Comments', 'briddge-addon' ) .' </span></a></p><p class="post-item-date"><a href="'.get_day_link( $archive_year, $archive_month, $archive_day).'" ><span>'. get_the_time( get_option('date_format') ) .'</span></a></p>';
					$tab1 .= '</div>';
					$tab1 .= '</div>';
					$tab1 .= '</li>';
				} //while 
	...
	function advanceTabWidgetCommentsOutput($args){
		...
		if ( is_array( $comments ) && $comments ) {
			foreach ( (array) $comments as $comment ) {
				...
				$output .= '<a href="' . esc_url( get_comment_link( $comment ) ) . '">' . get_the_title( $comment->comment_post_ID ) . '</a> #'.  get_comment_excerpt($comment).' '. esc_html__('by', 'briddge-addon') .' <span class="comment-author-link"><i>' . get_comment_author_link( $comment ) . '</i></span>' ;
				$output .= '</li>';
			}
		}
		$output .= '</ul>';
	...
	function form( $instance ) {
		...
		<!-- First TAB title -->
		<p><h4><u><?php esc_html_e('First Tab Settings:', 'briddge-addon'); ?></u></h4></p>
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'tabtitle1' ) ); ?>"><?php esc_html_e('First Tab Title:', 'briddge-addon'); ?></label>
		...
		<label for="<?php echo esc_attr( $this->get_field_id('categories1') ); ?>"><?php esc_html_e('Filter by Category:', 'briddge-addon'); ?></label> 
		<select id="<?php echo esc_attr( $this->get_field_id('categories1') ); ?>" name="<?php echo esc_attr( $this->get_field_name('categories1') ); ?>" class="widefat categories" style="width:100%;">
			<option value='all' <?php if ('all' == $instance['categories1']) echo 'selected="selected"'; ?>><?php esc_html_e('All categories', 'briddge-addon'); ?></option>
		...
		<p>
		<label for="<?php echo esc_attr( $this->get_field_id('selpost1') ); ?>"><?php esc_html_e('Select Post Type:', 'briddge-addon'); ?></label> 
		<select id="<?php echo esc_attr( $this->get_field_id('selpost1') ); ?>" name="<?php echo esc_attr( $this->get_field_name('selpost1') ); ?>" class="widefat advance_widget_posttab_selpost" style="width:100%;">
			<?php 
				$postviews = array(
					"recent"=> esc_html__( 'Recent Posts', 'briddge-addon' ),
					"mostlike"=> esc_html__( 'Most Like Posts', 'briddge-addon' ),
					"mostview"=> esc_html__( 'Most Viewed Posts', 'briddge-addon' ),
					"comments"=> esc_html__( 'Post Comments', 'briddge-addon' )
				); 
			?>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'number1' ) ); ?>"><?php esc_html_e('Number of Recent posts/comments to show:', 'briddge-addon'); ?></label>
		...
		<!-- Second TAB title -->
		<p><h4><u><?php esc_html_e('Second Tab Settings:', 'briddge-addon'); ?></u></h4></p>
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'tabtitle2' ) ); ?>"><?php esc_html_e('Second Tab Title:', 'briddge-addon'); ?></label>
		...
		<label for="<?php echo esc_attr( $this->get_field_id('categories2') ); ?>"><?php esc_html_e('Filter by Category:', 'briddge-addon'); ?></label> 
		<select id="<?php echo esc_attr( $this->get_field_id('categories2') ); ?>" name="<?php echo esc_attr( $this->get_field_name('categories2') ); ?>" class="widefat categories" style="width:100%;">
			<option value='all' <?php if ('all' == $instance['categories2']) echo 'selected="selected"'; ?>><?php esc_html_e('All categories', 'briddge-addon'); ?></option>
			<?php $categories = get_categories('hide_empty=1&depth=1&type=post'); ?>
			<?php foreach($categories as $category) { ?>
		...
		<!-- Select Post (Recent, Popular ...) -->
		<p>
		<label for="<?php echo esc_attr( $this->get_field_id('selpost2') ); ?>"><?php esc_html_e('Select Post Type:', 'briddge-addon'); ?></label> 
		...
		<!-- Number of posts/comments -->
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'number2' ) ); ?>"><?php esc_html_e('Number of Recent posts/comments to show:', 'briddge-addon'); ?></label>
		...
		<!-- Thrid TAB title -->
		<p><h4><u><?php esc_html_e('Thrid Tab Settings:', 'briddge-addon'); ?></u></h4></p>
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'tabtitle3' ) ); ?>"><?php esc_html_e('Thrid Tab Title:', 'briddge-addon'); ?></label>
		...
		<label for="<?php echo esc_attr( $this->get_field_id('categories3') ); ?>"><?php esc_html_e('Filter by Category:', 'briddge-addon'); ?></label> 
		<select id="<?php echo esc_attr( $this->get_field_id('categories3') ); ?>" name="<?php echo esc_attr( $this->get_field_name('categories3') ); ?>" class="widefat categories" style="width:100%;">
			<option value='all' <?php if ('all' == $instance['categories3']) echo 'selected="selected"'; ?>><?php esc_html_e('All categories', 'briddge-addon'); ?></option>
			<?php $categories = get_categories('hide_empty=1&depth=1&type=post'); ?>
			<?php foreach($categories as $category) { ?>
		...
		<!-- Select Post (Recent, Popular ...) -->
		<p>
		<label for="<?php echo esc_attr( $this->get_field_id('selpost3') ); ?>"><?php esc_html_e('Select Post Type:', 'briddge-addon'); ?></label> 
		...
		<!-- Number of posts/comments -->
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'number3' ) ); ?>"><?php esc_html_e('Number of Recent posts/comments to show:', 'briddge-addon'); ?></label>
