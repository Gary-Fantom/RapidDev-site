...
class briddge_wp_actions {
	...
	public function __construct() {
		...
		//Google fonts
		add_action( 'wp_head', array( $this, 'briddge_google_fonts_con' ), 10 );

		//Theme front scripts
		add_action( 'wp_enqueue_scripts', array( $this, 'briddge_front_scripts' ), 10 );

		//body class
		add_filter( 'body_class', array( $this, 'briddge_custom_class' ), 99 );

	}

	public static function briddge_google_fonts_con(){

		$g_arr = get_option( 'briddge_google_fonts_list' );
	...
	function briddge_front_scripts(){
		//Register font awesome styles
		wp_register_style( 'font-awesome', BRIDDGE_ADDON_URL . 'admin/extension/theme-options/assets/css/font-awesome.min.css', array(), '4.7.0', 'all' );
	}

	function briddge_custom_class( $classes ) {
		$briddge_options = get_option( 'briddge_options' ); 
		$rtl = isset( $briddge_options['rtl'] ) ? $briddge_options['rtl'] : false;
		if( $rtl ) {
			$classes[] = 'rtl';
		}
...
briddge_wp_actions::get_instance();